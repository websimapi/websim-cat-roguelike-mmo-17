...
export function checkCollision(x, y) {
    ...
    // 1. Map Boundaries
    // Allow entering the void at the side gates (GridH / 2)
    const gateY = Math.floor(CONFIG.GRID_H / 2);
    
    // Check bounds
    if (gy < 0 || gy >= CONFIG.GRID_H || gx < 0 || gx >= CONFIG.GRID_W) {
        // If we are exactly on the gate row, allow out-of-bounds (Void entry)
        if (gy === gateY) return false;
        return true;
    }
